...
import { emptyFunc } from 'src/lib/helpers/emptyFunc'

import { cn } from './Landing.style'

export const Landing = () => (
  <PageContainer dataTestAtr="page-landing">
    <GithubCorner />
    <div className={`${cn.header}`}>
      <div className={`text-right`}>
        <span className="pr-sm">
          <Link to={routes.signin()} className="text-no-underline">
            <Button onClick={emptyFunc} color="primary" size="lg" outlined>
              <span data-test="page-landing__button-signin">Sign in</span>
            </Button>
          </Link>
        </span>
        <Link to={routes.signup()} className="text-no-underline">
          <Button onClick={emptyFunc} color="primary" size="lg" filled>
            <span data-test="page-landing__button-signup">Sign up</span>
          </Button>
        </Link>
      </div>
    </div>
      ...
      <div className={cn.messages}>
        ...
        <div className={`${cn.message}`}>
          ...
          <div className="pb-sm">
            ...
            <a target="_blank" rel="noopener noreferrer" href="https://money.yandex.ru/to/410011439375102">
              Donate!
            </a>
          </div>
        </div>
